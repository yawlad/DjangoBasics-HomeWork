...
class CoursesDetailView(TemplateView):
    ...
    def get_context_data(self, pk=None, **kwargs):
        ...
        logger.debug(f"Opened CourseDetail {get_object_or_404(mainapp_models.Courses, pk=pk).name} by {self.request.user.username} with id: {self.request.user.id}")
...
# ---------------------------------------------
            ...
            # with open(f"mainapp/fixtures/005_feedback_list_{pk}.bin", "wb") as outfile:
...
class ContactsPageView(TemplateView):
    ...
    def post(self, *args, **kwargs):
        if self.request.user.is_authenticated:
            cache_lock_flag = cache.get(f"mail.feedback_lock_{self.request.user.pk}")
            if not cache_lock_flag:
                cache.set(
                    f"mail.feedback_lock_{self.request.user.pk}",
                    "lock",
                    timeout=300,
                )
        ...
        with open(settings.LOG_FILE, "rb") as log_file:
            try:
                ...
                for item in log_file.readlines():
                    log_slice.insert(0, item.decode())
            except:
                log_file.seek(0)
